...
const MechanicalScore: React.FC<{ score: number, size?: 'sm' | 'lg' }> = ({ score, size = 'lg' }) => {
  ...
  return (
    <div className="flex gap-[1px] sm:gap-[2px]">
      {digits.map((digit, i) => (
        <div key={i} className={`relative bg-[#121212]/80 flex items-center justify-center rounded-[3px] sm:rounded-[4px] border border-white/20 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] overflow-hidden ${size === 'lg' ? 'w-5 h-8 sm:w-8 sm:h-10' : 'w-4 h-6 sm:w-5 sm:h-7'}`}>
          <div className="absolute inset-x-0 top-1/2 h-px bg-black/50 shadow-[0_1px_0_rgba(255,255,255,0.1)] z-10" />
          <span className={`font-data font-black text-white relative z-0 ${size === 'lg' ? 'text-lg sm:text-2xl' : 'text-xs sm:text-sm'}`}>{digit}</span>
        </div>
      ))}
    </div>
  );
...
const TrendIndicator: React.FC<{ trend: string, val: number }> = ({ trend, val }) => {
  if (trend === 'up') return <div className="flex items-center gap-0.5 text-[#FFC800] bg-[#FFC800]/10 px-1 py-0.5 rounded-sm"><TrendingUp size={10} strokeWidth={3} /><span className="text-[9px] font-black">{val}</span></div>;
  if (trend === 'down') return <div className="flex items-center gap-0.5 text-[#FF4D1C] bg-[#FF4D1C]/10 px-1 py-0.5 rounded-sm"><TrendingDown size={10} strokeWidth={3} /><span className="text-[9px] font-black">{val}</span></div>;
  if (trend === 'new') return <div className="text-[8px] font-black uppercase text-[#FFC800] tracking-wider bg-[#FFC800]/10 px-1 py-0.5 rounded-sm">New</div>;
  return <div className="text-white/20"><Minus size={10} strokeWidth={3} /></div>;
};
    ...
    <PageLayout>
      <div className="min-h-screen relative flex flex-col w-full">
        ...
        <div className="max-w-4xl mx-auto px-2 sm:px-6 relative z-20 pt-2 w-full">
          <div className="bg-[#141414]/60 backdrop-blur-xl p-1 rounded-xl shadow-sm border border-white/10 flex max-w-xs mx-auto mb-6 relative">
            {(Object.keys(COMPETITION_CONFIG) as ScoringTrack[]).map((track) => {
              const isActive = activeTrack === track;
              return (
                <button key={track} onClick={() => setActiveTrack(track)} className={`flex-1 py-1.5 px-2 rounded-lg font-black uppercase text-[10px] tracking-widest relative z-10 transition-colors ${isActive ? 'text-black' : 'text-white/50 hover:text-white'}`}>
          ...
          <div className="flex items-end justify-center gap-1 sm:gap-4 mb-8 px-1 h-[200px] sm:h-[240px]">
            <AnimatePresence mode="popLayout">
              {podiumDisplay.map((player) => {
                const isFirst = player.rank === 1;
                const isSecond = player.rank === 2;
                const heightClass = isFirst ? 'h-40 sm:h-52' : isSecond ? 'h-32 sm:h-44' : 'h-28 sm:h-36';
                return (
                  <motion.div layout initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={springTransition} key={`podium-${player.name}-${activeTrack}`} className={`relative w-[32%] sm:w-40 bg-[#141414]/60 backdrop-blur-xl border border-white/10 rounded-t-2xl shadow-[0_10px_40px_rgba(0,0,0,0.4)] flex flex-col items-center pt-6 sm:pt-8 pb-3 sm:pb-4 px-1 sm:px-2 ${heightClass}`}>
                    {isFirst && (
                      <motion.div initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ delay: 0.3, type: 'spring' }} className="absolute -top-10 sm:-top-12 text-[#FFD700] drop-shadow-[0_0_15px_rgba(255,215,0,0.8)] flex flex-col items-center">
                        <Sparkles size={14} className="absolute -top-3 -right-3 text-[#FFD700]/80 animate-pulse" />
                        <Crown size={32} fill="currentColor" strokeWidth={1} />
                      </motion.div>
                    )}
                    <div className={`absolute -top-6 sm:-top-8 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center font-black text-lg sm:text-xl border-2 sm:border-4 border-[#141414] shadow-lg ${medalStyles[player.rank as 1|2|3]}`}>
                      {player.initials}
                    </div>
                    <div className="mt-6 sm:mt-8 flex flex-col items-center text-center w-full">
                      <span className="font-black text-[9px] sm:text-[10px] text-white/40 tracking-widest uppercase mb-0.5">Rank {player.rank}</span>
                      <h3 className={`font-black uppercase leading-tight truncate w-full px-1 ${isFirst ? 'text-xs sm:text-sm text-white' : 'text-[9px] sm:text-xs text-white/80'}`}>{player.name}</h3>
                    </div>
                    <div className="mt-auto flex flex-col items-center">
                      <span className="font-data font-bold text-[7px] sm:text-[8px] uppercase tracking-[0.2em] text-white/40 mb-0.5 sm:mb-1">Points</span>
          ...
          <div className="relative">
            <div className="flex items-center justify-between px-2 mb-2 border-b-2 border-dashed border-white/10 pb-1.5">
              <h3 className="font-black uppercase text-xs sm:text-sm tracking-[0.2em] text-white">The Pack</h3>
              <span className="font-data font-bold text-[8px] sm:text-[9px] uppercase tracking-widest text-white/60 bg-white/5 px-1.5 py-0.5 rounded shadow-sm border border-white/10">Pos 4 &mdash; {leaderboardData.length}</span>
            </div>
              ...
              <table className="w-full text-left border-separate table-fixed" style={{ borderSpacing: '0 4px' }}>
                <thead>
                  <tr className="text-[8px] sm:text-[9px] uppercase font-black text-white/40 tracking-widest">
                    <th className="px-2 pb-1 w-10 sm:w-14 text-center">#</th>
                    <th className="px-2 pb-1">Player</th>
                    <th className="px-2 pb-1 text-right hidden sm:table-cell w-14">BAs</th>
                    <th className="px-2 pb-1 text-right hidden sm:table-cell w-16">QELO</th>
                    <th className="px-2 pb-1 text-right w-14 sm:w-16">Score</th>
                  </tr>
                </thead>
                <tbody>
                  <AnimatePresence mode="popLayout">
                    {theRest.map((player) => (
                      <motion.tr layout variants={fadeUpItem} key={`pack-${player.name}-${activeTrack}`} className="group">
                        <td className="p-1 sm:p-1.5 text-center bg-[#141414]/60 backdrop-blur-xl border-y border-l border-white/10 rounded-l-lg shadow-[0_4px_15px_rgb(0,0,0,0.3)]">
                          <div className="mx-auto w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center font-data font-black text-[9px] sm:text-[10px] bg-white/10 text-white/90 border border-white/20">
                            {String(player.rank).padStart(2, '0')}
                          </div>
                        </td>
                        <td className="p-1.5 sm:p-2 min-w-0 bg-[#141414]/60 backdrop-blur-xl border-y border-white/10">
                          <div className="flex flex-col min-w-0">
                            <h4 className="font-black uppercase text-[10px] sm:text-xs text-white truncate group-hover:text-[#FF7A1A] transition-colors">{player.name}</h4>
                        ...
                        <td className="p-1.5 sm:p-2 text-right hidden sm:table-cell bg-[#141414]/60 backdrop-blur-xl border-y border-white/10">
                          <span className="font-data font-black text-[10px] sm:text-[11px] text-white/70">{player.bas}</span>
                        </td>
                        <td className="p-1.5 sm:p-2 text-right hidden sm:table-cell bg-[#141414]/60 backdrop-blur-xl border-y border-white/10">
                          <span className="font-data font-black text-[10px] sm:text-[11px] text-white/70">{player.qelo.toFixed(1)}</span>
                        </td>
                        <td className="p-1.5 sm:p-2 text-right pr-2 sm:pr-3 bg-[#141414]/60 backdrop-blur-xl border-y border-r border-white/10 rounded-r-lg shadow-[0_4px_15px_rgb(0,0,0,0.3)]">
                          <span className="font-data font-black text-[10px] sm:text-[11px] bg-white/20 text-white px-1.5 py-0.5 rounded shadow-inner">{player.pts}</span>
                        </td>
                      </motion.tr>
                    ))}
                  </AnimatePresence>
